...
# Konfigurasi Halaman
st.set_page_config(page_title="Model A (LSTM) - v1.0 Baseline", layout="wide")

st.title("🌡️ Model A: LSTM Temperature Forecaster (v1.0)")
...
if input_mode == "Manual Input":
    st.write("Masukkan nilai saat ini (akan diduplikasi menjadi 144 timesteps untuk simulasi baseline):")
    manual_data = {}
    manual_data['T (degC)'] = st.number_input("Suhu T (degC)", value=15.0)
    ...
    uploaded_file = st.file_uploader("Unggah file sample_jena.csv (minimal 144 baris)", type=['csv'])
...
if st.button("Predict 6-Hours Ahead Temperature", type="primary"):
    if input_sequence is None:
        st.warning("Silakan generate sequence input terlebih dahulu di atas!")
    else:
        with st.spinner("Memuat model tanpa cache (Simulasi Load Lambat)..."):
            ...
            try:
                ...
                st.markdown(f"### Suhu Terprediksi ($T_{{t+6h}}$): **{actual_temp:.2f} °C**")
                
            except Exception as e:
                st.error(f"Gagal memuat atau menjalankan model: {e}")
                st.error("Pastikan Anda sudah menginstall TensorFlow dan file lstm_config1.h5 tersedia di folder models.")
